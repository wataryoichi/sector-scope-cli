...
- **セクター管理を分離**: 価格は外部ソース、テーマ分類は自前YAML管理
...
Yahoo Finance 等のデータは便利だが、テーマ分類（軍事、農業、AI インフラなど）は安定して一貫提供されるとは限らない。そのため「価格は外部ソースから取得」「セクター所属は自前で管理」という構成を採用している。

## カスタムセクターの追加

`~/.sectorscope/universe/us/` にYAMLファイルを置くと、自分だけのカスタムセクターを追加できます。
